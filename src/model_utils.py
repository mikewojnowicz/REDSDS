...
def build_model(config):
    ...
    x_init = InitialContinuousDistribution(
        ...
        max_scale=config["initial_state"].get("max_scale", 1.0),
    ...
    x_transition = ContinuousStateTransition(
        ...
        takes_ctrl=config["control"]["x"],
        max_scale=config["continuous_transition"].get("max_scale", 1.0),
    ...
    emission_network = GaussianDistributionOutput(
        ...
        max_scale=config["emission"].get("max_scale", 1.0),
    ...
    posterior_dist = GaussianDistributionOutput(
        ...
        max_scale=config["inference"].get("max_scale", 1.0),
    ...
    if (
        ...
        if config["inference"]["use_causal_rnn"]:
            posterior_rnn = nn.RNNCell(
                emb_out_dim + config["x_dim"] + inf_ctrl_dim,
        ...
        posterior_mlp = nn.Sequential(
            nn.Linear(posterior_mlp_in_dim, config["inference"]["mlp_hiddendim"]),
            nn.ReLU(True),
        ...
        # Discrete Transition
        discrete_in_dim = (
            config["x_dim"] if config["discrete_transition"]["takes_x"] else 0
        )
        discrete_in_dim += (
            config["obs_dim"] if config["discrete_transition"]["takes_y"] else 0
        )
